...
export function limesToCut(wedgesNeeded, limes) {
  ...
  while (wedgesCounter < wedgesNeeded && limeCounter < limes.length) {
    switch (limes[limeCounter]) {
      case 'small':
        wedgesCounter += LIME_SIZES.small;
        limeCounter++;
        continue;

      case 'medium':
        wedgesCounter += LIME_SIZES.medium;
        limeCounter++;
        continue;

      case 'large':
        wedgesCounter += LIME_SIZES.large;
        limeCounter++;
        continue;
      default:
        break;
...
export function remainingOrders(timeLeft, orders) {
  ...
  // NOTE: is not a good practice to alter arguments of a function, do not use orders.shift()
  // look for functional programming fundamentals
