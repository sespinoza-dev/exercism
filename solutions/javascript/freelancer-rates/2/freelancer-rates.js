...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const normalValueForFullMonth = fullMonths * BILLABLE_DAYS_PER_MONTH * dayRate(ratePerHour);
  const hoursBilledWithDiscount = applyDiscount(normalValueForFullMonth, discount);

  return Math.ceil(hoursBilledWithoutDiscount + hoursBilledWithDiscount);
}

/**
 * Applies a discount rate to a value
 *
 * @param {number} value
 * @param {number} discount: for example 20% written as 0.2
 * @returns {number} the value with the discount
 */
